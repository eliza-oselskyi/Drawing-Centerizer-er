...
public abstract partial class QuickCenterClass
{
    ...
    private static void GenerateAndDisplayReport(string reportTemplate, string reportString)
    {
        ...
        var tempTemplate = File.Open( $@"{Directory.GetCurrentDirectory()}\report_template\{reportTemplate}_temp.rpt", FileMode.Create);
        var template = File.Open($@"{Directory.GetCurrentDirectory().ToString()}\report_template\{reportTemplate}_test.rpt", FileMode.Open);
        
        //tempTemplate.Close();
        template.CopyTo(tempTemplate);
        tempTemplate.Close();
        using (StreamWriter writer = File.AppendText(tempTemplate.Name))
        {
        ...
        Console.WriteLine(
            (Tekla.Structures.Model.Operations.Operation.CreateReportFromAll(reportTemplate, reportTemplate + ".xsr",
